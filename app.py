...
from utils.email_templates import category_label
...
try:
    app.config["USD_VND_RATE"] = float(os.environ.get("USD_VND_RATE", "25000"))
except ValueError:
    app.config["USD_VND_RATE"] = 25000.0
if app.config["USD_VND_RATE"] <= 0:
    app.config["USD_VND_RATE"] = 25000.0
...
class User(UserMixin, db.Model):
    ...
    preferred_language = db.Column(db.String(2), nullable=False, default="en")
    preferred_currency = db.Column(db.String(3), nullable=False, default="VND")
...
def user_preferences(user=None):
    user = user or (current_user if current_user.is_authenticated else None)
    language = getattr(user, "preferred_language", "en") if user else "en"
    currency = getattr(user, "preferred_currency", "VND") if user else "VND"
    ...
    return {
        "language": language,
        "currency": currency,
        "usd_vnd_rate": app.config["USD_VND_RATE"],
    }


@app.context_processor
def inject_app_preferences():
    return {"app_preferences": user_preferences()}
...
def deliver_verification_email(user):
    ...
    delivered = send_verification_email(
        ...
        usd_vnd_rate=preferences["usd_vnd_rate"],
    )
    if delivered:
...
def display_money(amount, preferences):
    if preferences["currency"] == "USD":
        return f'${float(amount) / preferences["usd_vnd_rate"]:,.2f}'
...
def build_state(user, period=None):
    ...
    if over_budget:
        ...
        notifications.append({
            "id": 1,
            "kind": "warn",
            "ago": "Now" if preferences["language"] == "en" else "Hiện tại",
            "text": (
                f'{category_label(b["category"], "en")} is over budget by {display_money(b["over"], preferences)}'
                if preferences["language"] == "en"
                else f'{category_label(b["category"], "vi")} đã vượt ngân sách {display_money(b["over"], preferences)}'
            ),
        })
    if transactions:
        notifications.append({
            "id": 2,
            "kind": "done",
            "ago": "Latest" if preferences["language"] == "en" else "Mới nhất",
            "text": (
                f'Transaction “{transactions[0].merchant}” was synced'
                if preferences["language"] == "en"
                else f'Đã đồng bộ giao dịch “{transactions[0].merchant}”'
            ),
        })
...
def coach_response_language(message, preferred_language="en"):
    """Use the question's language, falling back to the saved UI preference."""
    ...
    if re.search(r"[ăâđêôơưáàảãạấầẩẫậắằẳẵặéèẻẽẹếềểễệíìỉĩịóòỏõọốồổỗộớờởỡợúùủũụứừửữựýỳỷỹỵ]", normalized):
        return "vi"
    vietnamese_terms = (
        " bao nhieu ", " cua toi ", " toi da ", " chi tieu ", " mua sam ",
        " ngan sach ", " tai khoan ", " so du ", " muc tieu ", " tiet kiem ",
        " thang nay ", " tuan qua ",
    )
    if any(term in normalized for term in vietnamese_terms):
        return "vi"
    if re.search(r"\b(what|how|why|when|where|which|spent|spending|budget|balance|account|goal|save|month|week)\b", normalized):
        return "en"
    return preferred_language if preferred_language in {"en", "vi"} else "en"
...
def local_coach_reply(message, state):
    ...
    english = coach_response_language(message, preferences["language"]) == "en"
    money = lambda amount: display_money(amount, preferences)
    if any(word in query for word in ("balance", "account", "số dư", "so du", "tài khoản", "tai khoan")):
        items = "\n".join(f'- {a["name"]}: {money(a["balance"])}' for a in state["accounts"])
        return (
            f'**Current balances**\n\n{items}\n\n**Total:** {money(kpi["balance"])}.'
            if english else f'**Số dư hiện tại**\n\n{items}\n\n**Tổng cộng:** {money(kpi["balance"])}.'
        )
    if any(word in query for word in ("budget", "over", "ngân sách", "ngan sach", "vượt", "vuot")):
        worst = max(state["budgets"], key=lambda b: b["pct"], default=None)
        if not worst or not worst["spent"]:
            return "There is no spending to assess this month." if english else "Tháng này chưa có khoản chi nào để đánh giá ngân sách."
        if english:
            action = "Pause non-essential purchases in this category" if worst["over"] else "Keep the remaining spending below the limit"
    ...
    if any(word in query for word in ("mục tiêu", "muc tieu", "goal", "tiết kiệm", "tiet kiem")):
        if not state["goals"]:
            return "You do not have a savings goal yet. Create one with a target amount and deadline." if english else "Bạn chưa có mục tiêu tiết kiệm. Hãy tạo một mục tiêu có số tiền và thời hạn cụ thể."
        goal = min(state["goals"], key=lambda g: max(0, g["target"] - g["saved"]))
        remaining = max(0, goal["target"] - goal["saved"])
        return (
            f'“{goal["name"]}” needs {money(remaining)} more. Set aside a fixed amount after each income payment.'
            if english else f'“{goal["name"]}” còn {money(remaining)}. Hãy dành một khoản cố định ngay sau mỗi lần nhận thu nhập.'
        )
    if kpi["income"] <= 0:
        return (
            f'You have spent {money(kpi["expense"])} this month without recorded income. Add income so the savings rate is accurate.'
            if english else f'Tháng này bạn đã chi {money(kpi["expense"])} nhưng chưa ghi nhận thu nhập. Hãy thêm thu nhập để AI tính tỷ lệ tiết kiệm chính xác.'
        )
    if english:
        return f'This month you earned {money(kpi["income"])}, spent {money(kpi["expense"])}, and saved {kpi["savings_rate"]}%. Reduce the largest spending category first.'
    return (
...
def api_signup():
    ...
    preferred_currency = request.form.get("preferred_currency", "VND")
    if preferred_language not in {"en", "vi"}:
        preferred_language = "en"
    if preferred_currency not in {"VND", "USD"}:
...
def update_account_preferences():
    ...
    if "preferred_language" in data:
        if data["preferred_language"] not in {"en", "vi"}:
            return json_error("Ngôn ngữ không hợp lệ.")
        current_user.preferred_language = data["preferred_language"]
    if "preferred_currency" in data:
        if data["preferred_currency"] not in {"VND", "USD"}:
            return json_error("Đơn vị tiền tệ không hợp lệ.")
...
def manage_funding_source(account_id):
    ...
    account = Account.query.filter_by(id=account_id, user_id=current_user.id).first()
    if not account:
        return json_error(source_message("Funding source not found.", "Không tìm thấy nguồn tiền."), 404)
    ...
    if request.method == "DELETE":
        if Account.query.filter_by(user_id=current_user.id).count() <= 1:
            return json_error(source_message(
                "You must keep at least one funding source.",
                "Bạn cần giữ lại ít nhất một nguồn tiền.",
            ))
        ...
        try:
            ...
            Goal.query.filter_by(
                user_id=current_user.id, account_id=account.id
            ).update({Goal.account_id: None}, synchronize_session=False)
            db.session.delete(account)
            db.session.commit()
        except Exception:
            db.session.rollback()
            app.logger.exception("Funding source deletion failed")
            return json_error(source_message(
                "Unable to delete the funding source right now. Please try again.",
                "Không thể xóa nguồn tiền lúc này. Vui lòng thử lại.",
            ), 500)

        return jsonify({
            "success": True,
            "deleted_transactions": deleted_transactions,
            "state": build_state(current_user, period),
        })
    ...
    account.name = name
    account.opening_balance = round(raw_balance) - account_transaction_delta(account)
    db.session.commit()
    return jsonify({
        "success": True,
        "state": build_state(current_user, period),
...
def add_transaction():
    ...
    if tx_type == "expense" and budget and budget["over"] > 0:
        ...
        if (
            ...
            try:
                email_alert_sent = bool(send_budget_alert_email(
                    current_user.email,
    ...
    return jsonify({
        ...
        "state": state,
        "breachAlert": breach,
        "emailAlertSent": email_alert_sent,
    }), 201
...
def coach():
    ...
    if len(message) > 2_000:
        return json_error("Câu hỏi quá dài. Vui lòng giới hạn trong 2.000 ký tự.")
    state = build_state(current_user)
    preferences = state["preferences"]
    response_language = coach_response_language(message, preferences["language"])
    ...
    if client:
        try:
            ...
            language_instruction = (
                "Respond in English because the latest question is in English."
                if response_language == "en"
                else "Respond in Vietnamese because the latest question is in Vietnamese."
            )
            currency_instruction = (
                f"All monetary values in the JSON are base VND. Convert them to USD using "
                f"1 USD = {preferences['usd_vnd_rate']:g} VND and display USD with two decimals."
                if preferences["currency"] == "USD"
                else "All monetary values in the JSON are VND. Display them as VND without decimal places."
            )
            response = client.responses.create(
                ...
                instructions=(
                    f"You are a personal finance assistant. {language_instruction} {currency_instruction} "
                    "Use only the supplied data and never invent numbers. Keep the answer under 120 words "
                    "and give 1-3 concrete actions when useful. Use simple Markdown with short paragraphs "
                    "and lists. Put every list item on its own line. Never use HTML or BBCode, and always "
                    "close Markdown emphasis markers."
                ),
                input=(
                    f"Financial data JSON: {json.dumps(compact, ensure_ascii=False)}\n"
    ...
    return jsonify({
        "reply": local_coach_reply(message, state),
        "source": "local",
        "format": "markdown",
        "language": response_language,
    })
